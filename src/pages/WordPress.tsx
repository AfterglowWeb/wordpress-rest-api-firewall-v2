...
function FileActionSwitch({
	...
	const runAction = useCallback(async () => {
		...
		try {
			...
			setResult({
				success: true,
				message: response?.message || __('Done.', 'rest-api-firewall'),
			});
		} catch (err) {
			setResult({ 
				success: false, 
				message: err instanceof Error ? err.message : __('An error occurred.', 'rest-api-firewall') 
			});
		} finally {
			setBusy(false);
		}
...
export default function WordPress() {
	...
	const handleSave = useCallback(async () => {
		const confirmed = await new Promise<boolean>((resolve) => {
			openDialog({
				type: DIALOG_TYPES.CONFIRM,
				title: __('Save Security Settings', 'rest-api-firewall'),
				content: __('Save global security settings?', 'rest-api-firewall'),
		...
		try {
			...
			openDialog({
				...
				title: __('Global Security Saved', 'rest-api-firewall'),
				content: __('Global security settings saved successfully.', 'rest-api-firewall'),
			...
			openDialog({
				...
				content: error instanceof Error ? error.message : __('Failed to save settings.', 'rest-api-firewall'),
				confirmLabel: __('OK', 'rest-api-firewall'),
				onConfirm: closeDialog,
			});
		}
				...
				<Paper sx={{p:2}} elevation={0}>
                    ...
                    <Stack>
                        <Typography variant="h6">
                            {__('Redirect Front', 'rest-api-firewall')}
                        </Typography>

                        <Typography variant="caption" color="text.secondary">
                            {__('Turn WordPress into an Application resource only', 'rest-api-firewall')}
                        </Typography>
                    </Stack>
					<FormControl>
						<FormControlLabel
					...
					>
						...
						>
							<FormControlLabel
								...
								label={__('404 Not Found Response', 'rest-api-firewall')}
							/>
							<FormControlLabel
								value="login"
								control={<Radio size="small" />}
								label={__('WordPress Login URL', 'rest-api-firewall')}
							/>
							<FormControlLabel
								value="custom"
								control={<Radio size="small" />}
							...
							<Box sx={{ pl: 4, mt: 1 }}>
								<TextField
									...
									fullWidth
									helperText={__('Full URL with protocol and domain', 'rest-api-firewall')}
								/>
							</Box>
						</RadioGroup>
					</FormControl>
				...
				<Paper sx={{p:2}} elevation={0}>
					...
					<Box>
						...
						<FormHelperText sx={{ mt: 0, mb: 1 }}>
							{__('Add the following constant to your wp-config.php to disable the theme editor in WordPress admin.', 'rest-api-firewall')}
						</FormHelperText>
						...
						{securitySettings?.disallow_file_edit ? (
							<Alert severity="success" sx={{ fontSize: '0.75rem' }}>
								{__('DISALLOW_FILE_EDIT is defined and active.', 'rest-api-firewall')}
							</Alert>
						) : (
							<Alert severity="warning" sx={{ fontSize: '0.75rem' }}>
								{__('Constant not detected — editor is currently accessible.', 'rest-api-firewall')}
							</Alert>
						)}
					</Box>
