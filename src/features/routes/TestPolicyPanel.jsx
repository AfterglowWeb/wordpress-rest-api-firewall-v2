...
export default function TestPolicyPanel( {
	...
	const runTest = async () => {
		...
		try {
			...
			if ( result?.success ) {
				...
				setError(
					result?.data?.message ||
						__( 'Test failed', 'bromate-rest-api-firewall' )
				);
			}
		} catch ( err ) {
			setError(
				err.message || __( 'Network error', 'bromate-rest-api-firewall' )
			);
		} finally {
			setLoading( false );
		}
	...
	const getTestLabel = ( name ) => {
		switch ( name ) {
			case 'disabled':
				return __( 'Disabled', 'bromate-rest-api-firewall' );
			case 'auth':
				return __( 'Auth', 'bromate-rest-api-firewall' );
			case 'rate_limit':
				return __( 'Rate limit', 'bromate-rest-api-firewall' );
			default:
				return name;
		}
	};
	...
	const renderTestStatus = ( test ) => {
		if ( test?.skip ) {
			return (
				<Typography variant="caption" color="text.disabled">
					{ __( 'N/A', 'bromate-rest-api-firewall' ) }
				</Typography>
			);
		}
		if ( test?.pass === true ) {
			return (
				<Typography
					...
					fontWeight={ 600 }
				>
					{ __( 'Pass', 'bromate-rest-api-firewall' ) }
				</Typography>
			);
		}
		if ( test?.pass === null ) {
		...
		return (
			<Typography
				...
				fontWeight={ 600 }
			>
				{ __( 'Fail', 'bromate-rest-api-firewall' ) }
			</Typography>
		);
	};
				...
				{ results.map( ( result, index ) => (
					<Box key={ index }>
							...
							<Chip
								label={
									result.policy.protect
										? __( 'Protected', 'bromate-rest-api-firewall' )
										: __( 'Public', 'bromate-rest-api-firewall' )
								}
								size="small"
								variant="outlined"
							/>
						...
						<Stack direction="row" spacing={ 1.5 }>
							<DataPanel
								label={ __( 'Raw', 'bromate-rest-api-firewall' ) }
								data={ result.raw_data }
								downloadFilename={ `raw-${ result.method }-${ result.route.replace( /\//g, '-' ) }.json` }
								bgcolor="grey.50"
							/>
							<DataPanel
								label={ __( 'Result', 'bromate-rest-api-firewall' ) }
								...
								<Stack direction="row" spacing={ 1.5 } sx={ { mt: 1.5 } }>
									{ headersSent && (
										<Box sx={ { flex: 1, minWidth: 0 } }>
											<Stack direction="row" spacing={ 1 } alignItems="center" sx={ { mb: 0.5 } }>
												<Typography variant="caption" fontWeight={ 600 } sx={ { flex: 1 } }>
													{ __( 'Headers sent', 'bromate-rest-api-firewall' ) }
												</Typography>
									...
									{ curlSnippet && (
										<Box sx={ { flex: 1, minWidth: 0 } }>
											<Stack direction="row" spacing={ 1 } alignItems="center" sx={ { mb: 0.5 } }>
												<Typography variant="caption" fontWeight={ 600 } sx={ { flex: 1 } }>
													{ __( 'Replicate with curl', 'bromate-rest-api-firewall' ) }
												</Typography>
			...
			>
				...
				>
					...
					<Stack direction="row" gap={ 2 } alignItems="center">
						...
						>
						{ loading
							? __( 'Running…', 'bromate-rest-api-firewall' )
						: __( 'Re-run', 'bromate-rest-api-firewall' ) }
						</Button>
							...
							{ results && (
								<>
									...
									<IconButton
										...
										title={ __( 'Expand results', 'bromate-rest-api-firewall' ) }
									>
