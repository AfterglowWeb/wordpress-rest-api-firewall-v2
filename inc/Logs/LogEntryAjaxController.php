<?php namespace Bromate\RestApiFirewall\Logs;
...
class LogEntryAjaxController {
	...
	public function ajax_delete_log_entry(): void {
		...
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'Log ID required', 'bromate-rest-api-firewall' ) ), 400 );
		}

		$deleted = LogRepository::delete( $id );

		if ( ! $deleted ) {
			wp_send_json_error( array( 'message' => __( 'Log entry not found', 'bromate-rest-api-firewall' ) ), 404 );
		}
