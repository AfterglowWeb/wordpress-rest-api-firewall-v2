...
class IpBlacklistAjaxController {

	private function __construct() {}

	public static function register(): void {
		$self = new self();
		add_action( 'wp_ajax_get_global_ip_filter', array( $self, 'ajax_get_global_ip_filter' ) );
		add_action( 'wp_ajax_save_global_ip_filter', array( $self, 'ajax_save_global_ip_filter' ) );
	}
	...
	public function ajax_get_global_ip_filter(): void {
		...
		$options = IpBlacklistRepository::get_options();
	...
	public function ajax_save_global_ip_filter(): void {
		...
		$updated = IpBlacklistRepository::update_options( $options );
	...
	public function ajax_add_ip_entry(): void {
		...
		if ( empty( $ip ) || ! CidrMatcher::is_valid_ip_or_cidr( $ip ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid IP address', 'bromate-rest-application-layer' ) ), 400 );
		}

		if ( IpEntryRepository::find_by_ip( $ip, $list_type ) ) {
			wp_send_json_error( array( 'message' => __( 'IP already in list', 'bromate-rest-application-layer' ) ), 400 );
		}

		$expiry_seconds = (int) IpBlacklistRepository::get_options()['expiry_seconds'];
		...
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'Failed to add IP entry', 'bromate-rest-application-layer' ) ), 500 );
		}
	...
	public function ajax_delete_ip_entry(): void {
		...
		if ( ! $id ) {
			wp_send_json_error( array( 'message' => __( 'Entry ID required', 'bromate-rest-application-layer' ) ), 400 );
		}

		$deleted = IpEntryRepository::delete( $id );

		if ( ! $deleted ) {
			wp_send_json_error( array( 'message' => __( 'Entry not found', 'bromate-rest-application-layer' ) ), 404 );
		}
	...
	public function ajax_delete_ip_entries(): void {
		...
		if ( ! is_array( $ids ) || empty( $ids ) ) {
			wp_send_json_error( array( 'message' => __( 'No entries selected', 'bromate-rest-application-layer' ) ), 400 );
		}
