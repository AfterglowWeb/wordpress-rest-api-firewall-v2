	...
	public function check_before_auth( $user, $username, $password ) {
		...
		if ( empty( $options['site_key'] ) || empty( $options['secret_key'] ) ) {
			return new WP_Error(
				...
				__( 'reCAPTCHA is not properly configured. Please contact the site administrator.', 'bromate-rest-api-firewall' )
			);
		}
		...
		if ( empty( $token ) ) {
			return new WP_Error(
				...
				__( 'reCAPTCHA verification failed. Please try again.', 'bromate-rest-api-firewall' )
			);
		}
	...
	private function verify_recaptcha_token( string $token, string $secret_key ) {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				__( 'Unable to verify reCAPTCHA. Please try again.', 'bromate-rest-api-firewall' )
			);
		}
		...
		if ( ! isset( $data['success'] ) || ! $data['success'] ) {
			...
			return new WP_Error(
				...
				sprintf(
					__( 'reCAPTCHA verification failed. Error: %s', 'bromate-rest-api-firewall' ),
					$error_codes
				)
			);
		}
		...
		if ( isset( $data['action'] ) && $data['action'] !== 'login' ) {
			return new WP_Error(
				...
				__( 'Invalid reCAPTCHA action.', 'bromate-rest-api-firewall' )
			);
		}
