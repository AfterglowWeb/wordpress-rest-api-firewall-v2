...
class SaltRotation {
	...
	const OPTION_KEY        = 'bromate_rest_api_firewall_rotating_salts';
	const CONFIG_OPTION_KEY = 'bromate_rest_api_firewall_salt_rotation_config';
	const LAST_ROTATION_KEY = 'bromate_rest_api_firewall_last_salt_rotation';
	const CRON_HOOK         = 'bromate_rest_api_firewall_rotate_salts_event';

	const SCHEDULE_WEEKLY  = 'bromate_rest_api_firewall_weekly';
	const SCHEDULE_MONTHLY = 'bromate_rest_api_firewall_monthly';
	...
	public static function register(): void {
		...
		if ( empty( SettingsRepository::read_option('cookie_hardening_salt_rotation_enabled') ) ) {
			return; 
		}
		...
		add_action( 'wp_ajax_bromate_rest_api_firewall_salt_rotation_status', array( self::class, 'ajax_salt_rotation_status' ) );
		add_action( 'wp_ajax_bromate_rest_api_firewall_rotate_salt_now', array( self::class, 'ajax_rotate_salt_now' ) );
	...
	public static function register_custom_schedules( array $schedules ): array {
		if ( ! isset( $schedules[ self::SCHEDULE_WEEKLY ] ) ) {
			$schedules[ self::SCHEDULE_WEEKLY ] = array(
				...
				'display'  => __( 'Once Weekly (Bromate)', 'bromate-rest-api-firewall' ),
			);
		}
		if ( ! isset( $schedules[ self::SCHEDULE_MONTHLY ] ) ) {
			$schedules[ self::SCHEDULE_MONTHLY ] = array(
				'interval' => 30 * DAY_IN_SECONDS,
				'display'  => __( 'Once Monthly (Bromate)', 'bromate-rest-api-firewall' ),
			);
		}
		return $schedules;
	}
	...
	public static function sync_schedule(): void {
		...
		$enabled  = ! empty( $settings['cookie_hardening_salt_rotation_enabled'] );
		...
		$recurrence = self::sanitize_recurrence(
			$settings['cookie_hardening_salt_rotation_recurrence'] ?? 'week'
		);
		$time = self::sanitize_time(
			$settings['cookie_hardening_salt_rotation_time'] ?? '03:00'
		);
	...
	public static function ajax_rotate_salt_now(): void {
		...
		wp_send_json_success(
			array(
				'message' => __( 'Salt keys rotated. All sessions have been invalidated.', 'bromate-rest-api-firewall' ),
			),
			200
		);
	}
