...
final class TOTPLoginService {
    ...
    private const LOGIN_VERIFIED_META_KEY_PREFIX = '_bromate_totp_login_verified_';
    ...
    public static function register(): void {
        ...
        add_action('wp_login', array($service, 'handle_login_actions'), 10, 2);
    ...
    private function get_verified_meta_key(string $session_id): string {
        return self::LOGIN_VERIFIED_META_KEY_PREFIX . $session_id;
    }
    ...
    public function enqueue_scripts(): void {
        ...
        wp_localize_script('bromate-totp-login', 'bromateTotp', array(
    ...
    private function should_show_totp(): bool {
        ...
        $verified_meta_key = $this->get_verified_meta_key($session_id);
        if (get_user_meta($user_id, $verified_meta_key, true)) {
            return false;
        }
    ...
    private function render_totp_field(string $session_id): void {
        ...
        <div id="bromate-totp-field" class="bromate-totp-field">
            <h3 style="text-align:center;margin-bottom:12px">
                <?php esc_html_e('Two-factor Authentication', 'bromate-rest-api-firewall'); ?>
            </h3>
            <p>
                <label for="bromate-totp-code" style="display: block; margin-bottom: 4px; font-weight: 500;">
                    <?php esc_html_e('6-digit code on your app', 'bromate-rest-api-firewall'); ?>
                </label>
                <input 
                    type="text" 
                    ...
                    placeholder="<?php esc_attr_e('123456', 'bromate-rest-api-firewall'); ?>"
            ...
            <p class="forgetmenot" style="margin: 12px 0;">
                <label for="bromate-totp-remember" style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 500;">
                    ...
                    <?php esc_html_e('Remember this device for 30 days', 'bromate-rest-api-firewall'); ?>
                </label>
            </p>
    ...
    public function ajax_finish_login(): void {
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'bromate_totp_verify')) {
    ...
    public function validate_totp($user) {
        ...
        if (get_user_meta($user->ID, $verified_meta_key, true)) {
            return $user;
        }
        ...
        if (empty($_POST['bromate_totp_code'])) {
            ...
            return new WP_Error(
                ...
                __('Two-factor authentication is enabled for your account. Please enter your verification code.', 'bromate-rest-api-firewall')
            );
        }

        return new WP_Error(
            'bromate_totp_required',
            __('Please verify your code using the button below.', 'bromate-rest-api-firewall')
        );
    }
    ...
    public function ajax_verify_totp(): void {
        ...
        $code = isset($_POST['code']) ? preg_replace('/[^0-9]/', '', $_POST['code']) : '';
        if (strlen($code) !== 6) {
        ...
        $verified_meta_key = $this->get_verified_meta_key($session_id);
        update_user_meta($user_id, $verified_meta_key, true);
        
        if (isset($_POST['remember_device']) && filter_var($_POST['remember_device'], FILTER_VALIDATE_BOOLEAN)) {
    ...
    public function handle_login_actions($user_login, $user): void {
        $session_id = $this->get_session_id();
        delete_transient('bromate_totp_pending_' . $session_id);
        delete_transient('bromate_totp_attempts_' . $session_id);
    }
