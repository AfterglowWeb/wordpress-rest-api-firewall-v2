...
class HttpHeaders {
	...
	private function __construct() {
		if ( SettingsRepository::read_option( 'theme_secure_http_headers' ) ) {
			add_filter( 'rest_pre_serve_request', array( $this, 'add_security_headers' ), 10, 0 );
			if ( SettingsRepository::read_option( 'theme_wp_http_headers' ) ) {
				add_filter( 'send_headers', array( $this, 'add_security_headers' ), 10, 0 );
			}
		}
		if ( SettingsRepository::read_option( 'theme_compression_http_headers' ) ) {
